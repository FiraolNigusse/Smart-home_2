                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('rules.update', $rule) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $rule->name) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            @error('name')
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ old('description', $rule->description) }}</textarea>
                            @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="role_id" id="role_id" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                ...
                                <option value="{{ $role->id }}" {{ old('role_id', $rule->role_id) == $role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('role_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="device_id" id="device_id" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                ...
                                <option value="{{ $device->id }}" {{ old('device_id', $rule->device_id) == $device->id ? 'selected' : '' }}>{{ $device->name }}</option>
                                @endforeach
                            </select>
                            @error('device_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="action" id="action" value="{{ old('action', $rule->action) }}" placeholder="e.g., unlock, turn_on" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            @error('action')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="effect" id="effect" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                ...
                                <option value="deny" {{ old('effect', $rule->effect) === 'deny' ? 'selected' : '' }}>Deny</option>
                            </select>
                            @error('effect')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-4">
                            <label for="denial_message" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Denial Message</label>
                            <textarea name="denial_message" id="denial_message" rows="2" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ old('denial_message', $rule->denial_message) }}</textarea>
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $rule->is_active) ? 'checked' : '' }} class="rounded border-gray-300 dark:border-gray-700">
